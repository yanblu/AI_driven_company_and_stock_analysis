...
This document satisfies the guideline requirement to "clearly document your data sources and any preprocessing steps." It is updated as collectors run; the companion [`manifest.csv`](manifest.csv) carries per-artifact provenance (URL, retrieval date, SHA-256, record count).
...
| TD IR reports (Report to Shareholders) | 15 quarterly PDFs | FY2021–FY2025 × Q1/Q2/Q3 (Q4 is the 40-F) |
...
See the live provenance log in [`manifest.csv`](manifest.csv) for every file.
...
- **Output**: `data/raw/prices/{ticker}.parquet`
...
- **Output**: `data/raw/macro/{label}.parquet`
...
- **Output**: `data/raw/td_ir_reports/annual/fy{YYYY}_form_40f.{pdf,txt}`
...
- **Collected**: FY2021–FY2025 × Q1/Q2/Q3 = 15 PDFs (~350–425K chars each). Q4 is not published separately — it's absorbed into the 40-F.
- **Equivalent to**: US 10-Q (unaudited interim results + MD&A)
- **Output**: `data/raw/td_ir_reports/quarterly/{YYYY}_q{n}_report_to_shareholders.{pdf,txt}`
...
- **Output**: `data/raw/transcripts/{yyyy-mm-dd}_{slug}.{pdf,txt}` + `index.parquet`
...
- **Output**: `data/raw/news/{yyyy-mm-dd}_{slug}.{html,json}` + `index.parquet`
...
| `reports_quarterly.jsonl` | 15 | 1,086 | 1,479,278 | 1,363 |
...
- Output: JSONL per source type in `data/processed/llm_annotations/` (directory scaffolded, no annotations yet).
